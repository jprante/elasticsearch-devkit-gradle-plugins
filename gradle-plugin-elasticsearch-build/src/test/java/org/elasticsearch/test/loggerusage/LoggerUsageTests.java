...
public class LoggerUsageTests {

    private static final Logger logger = LogManager.getLogger(LoggerUsageTests.class);
        ...
        for (Method method : getClass().getMethods()) {
            if (method.getDeclaringClass().equals(getClass())) {
                if (method.getName().startsWith("check")) {
                    ...
                    List<LoggerUsageChecker.WrongLoggerUsage> errors = new ArrayList<>();
                    LoggerUsageChecker.check(errors::add, classInputStream,
    ...
    public void testLoggerUsageCheckerCompatibilityWithLog4j2Logger() {
        for (Method method : Logger.class.getMethods()) {
            if (LoggerUsageChecker.LOGGER_METHODS.contains(method.getName())) {
        ...
        for (String methodName : LoggerUsageChecker.LOGGER_METHODS) {
